...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class FogWithNoiseRender : PostProcessEffectRenderer<FogWithNoise>
    {
        ...
        {
            var sheet = context.propertySheets.Get(Shader.Find("Shaders/Fog/FogWithNoise"));
            ...
            sheet.properties.SetMatrix(Shader.PropertyToID("_FrustumCornersRay"), frustumCorners);

            sheet.properties.SetFloat(Shader.PropertyToID("_FogDensity"), settings.fogDensity * 2);
            sheet.properties.SetColor(Shader.PropertyToID("_FogColor"), settings.fogColor);
            sheet.properties.SetFloat(Shader.PropertyToID("FogStart"), settings.fogStart + context.camera.transform.position.y + 128);
            sheet.properties.SetFloat(Shader.PropertyToID("_FogEnd"), settings.fogEnd + context.camera.transform.position.y + 128);
            if (settings.noiseTexture.value != null)
            {
                sheet.properties.SetTexture(Shader.PropertyToID("_NoiseTex"), settings.noiseTexture.value);
            }
            sheet.properties.SetFloat(Shader.PropertyToID("_NoiseTex"), settings.fogXSpeed);
            sheet.properties.SetFloat(Shader.PropertyToID("_FogYSpeed"), settings.fogYSpeed);
            sheet.properties.SetFloat(Shader.PropertyToID("_NoiseAmount"), settings.noiseAmount);
            //material.SetFloat(ShaderIDs.WeakFactor", weakFactor);
            sheet.properties.SetFloat(Shader.PropertyToID("_OutArea"), settings.area * 2);
            sheet.properties.SetFloat(Shader.PropertyToID("_InArea"), settings.inArea / 2);
            sheet.properties.SetFloat(Shader.PropertyToID("_CenterX"), settings.centerX);
            sheet.properties.SetFloat(Shader.PropertyToID("_CenterY"), settings.centerY);
