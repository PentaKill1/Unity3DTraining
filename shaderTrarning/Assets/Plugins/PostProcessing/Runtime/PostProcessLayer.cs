...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class PostProcessLayer : MonoBehaviour
    {
        ...
        public delegate void CreatePost(int key, PostProcessLayer layer);
        public delegate void DestroyPost(int key, int opId);
        public static CreatePost createPostCall;
        public static DestroyPost destroyPostCall;
        public static Dictionary<int, PostProcessLayer> layers = new Dictionary<int, PostProcessLayer>();
 private void Awake()
        {
            int id = GetInstanceID();
            createPostCall?.Invoke(id, this);
        }

        private void OnDestroy()
        {
            int id = GetInstanceID();
            destroyPostCall?.Invoke(id, 0);
        }
        ...
        int RenderBuiltins(PostProcessRenderContext context, bool isFinalPass, int releaseTargetAfterUse = -1, int eye = -1)
        {
            ...
            RenderEffect<Glitch>(context);
            RenderEffect<FogWithNoise>(context);
