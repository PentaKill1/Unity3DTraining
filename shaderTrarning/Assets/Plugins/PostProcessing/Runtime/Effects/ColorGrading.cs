...
namespace UnityEngine.Rendering.PostProcessing {
	...
	public sealed class ColorGrading : PostProcessEffectSettings {
		...
		[DisplayName( "Mode" ),
		 Tooltip(
			 "Select a color grading mode that fits your dynamic range and workflow. Use HDR if your camera is set to render in HDR and your target platform supports it. Use LDR for low-end mobiles or devices that don't support HDR. Use External if you prefer authoring a Log LUT in an external software." )]
		...
		/// </remarks>
		[DisplayName( "Toe Strength" ), Range( 0f, 1f ),
		 Tooltip(
		...
		/// This is only used when <see cref="Tonemapper.Custom"/> is active.
		/// </remarks>
		[DisplayName( "Toe Length" ), Range( 0f, 1f ),
		 Tooltip(
			 "Affects how much of the dynamic range is in the toe. With a small value, the toe will be very short and quickly transition into the linear section, with a larger value, the toe will be longer." )]
		...
		/// This is only used when <see cref="Tonemapper.Custom"/> is active.
		/// </remarks>
		[DisplayName( "Shoulder Strength" ), Range( 0f, 1f ),
		 Tooltip(
		...
		/// This is only used when <see cref="Tonemapper.Custom"/> is active.
		/// </remarks>
		[DisplayName( "Shoulder Length" ), Min( 0f ),
		...
		/// This is only used when <see cref="Tonemapper.Custom"/> is active.
		/// </remarks>
		[DisplayName( "Shoulder Angle" ), Range( 0f, 1f ),
		...
		[DisplayName( "Lookup Texture" ),
		 Tooltip(
			 "Custom lookup texture (strip format, for example 256x16) to apply before the rest of the color grading operators. If none is provided, a neutral one will be generated internally." )]
		public TextureParameter ldrLut = new TextureParameter
			{value = null, defaultState = TextureParameterDefault.Lut2D}; // LDR only
		...
		/// This is only used when working with <see cref="GradingMode.LowDefinitionRange"/>.
		/// </remarks>
		[DisplayName( "Contribution" ), Range( 0f, 1f ),
		...
		/// </summary>
		[DisplayName( "Temperature" ), Range( -100f, 100f ),
		...
		/// </summary>
		[DisplayName( "Tint" ), Range( -100f, 100f ),
		...
		[DisplayName( "Post-exposure (EV)" ),
		 Tooltip(
			 "Adjusts the overall exposure of the scene in EV units. This is applied after the HDR effect and right before tonemapping so it won't affect previous effects in the chain." )]
		...
		/// </summary>
		[DisplayName( "Contrast" ), Range( -100f, 100f ),
		...
		/// </summary>
		[DisplayName( "Red" ), Range( -200f, 200f ),
		...
		/// </summary>
		[DisplayName( "Green" ), Range( -200f, 200f ),
		...
		/// </summary>
		[DisplayName( "Blue" ), Range( -200f, 200f ),
		...
		public SplineParameter masterCurve = new SplineParameter {
			value = new Spline( new AnimationCurve( new Keyframe( 0f, 0f, 1f, 1f ), new Keyframe( 1f, 1f, 1f, 1f ) ),
	...
	internal sealed class ColorGradingRenderer : PostProcessEffectRenderer<ColorGrading> {
		...
		void RenderExternalPipeline3D(PostProcessRenderContext context) {
			...
			uberSheet.properties.SetFloat( ShaderIDs.PostExposure,
				RuntimeUtilities.Exp2( settings.postExposure.value ) );
			context.logLut = lut;
		}

		private bool m_LutCached;
		private bool m_ForceLutUpdate;

		public void SetLutCacheDirty() {
			m_LutCached = false;
		}

		public void SetEnableForceLutUpdate( bool enabled ) {
			m_ForceLutUpdate = enabled;
		}
		...
		void RenderHDRPipeline3D(PostProcessRenderContext context) {
			...
			if( !m_LutCached || Application.isEditor || m_ForceLutUpdate ) {
				m_LutCached = true;
				CheckInternalLogLut();
				// Lut setup
				...
				cmd.SetComputeVectorParam( compute, "_Size",
					new Vector4( k_Lut3DSize, 1f / ( k_Lut3DSize - 1f ), 0f, 0f ) );
				...
				cmd.SetComputeVectorParam( compute, "_InvGamma",
					new Vector4( invgamma.x, invgamma.y, invgamma.z, 0f ) );
		...
		void CheckInternalLogLut() {
			...
			if( m_InternalLogLut == null || !m_InternalLogLut.IsCreated() ) {
				...
				m_InternalLogLut =
					new RenderTexture( k_Lut3DSize, k_Lut3DSize, 0, format, RenderTextureReadWrite.Linear ) {
